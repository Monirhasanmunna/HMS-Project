@extends('layouts.main')

@push('css')
    <!-- DataTables -->
  <link rel="stylesheet" href="{{asset('backend/plugins/datatables-bs4/css/dataTables.bootstrap4.min.css')}}">
  <link rel="stylesheet" href="{{asset('backend/plugins/datatables-responsive/css/responsive.bootstrap4.min.css')}}">
  <link rel="stylesheet" href="{{asset('backend/plugins/datatables-buttons/css/buttons.bootstrap4.min.css')}}">

    <style>
        table, th, td {
        border: 1px solid black;
        border-collapse: collapse;
        }
        th, td {
        padding: 5px;
        text-align: left;
        }



        @media print {
  body * {
    visibility: hidden;
  }
  .page * {
    visibility: visible;
  }
  #printBtn{
    visibility: hidden!important;
  }

}
    </style>
@endpush

@section('content')

<div class="content-wrapper">


<div class="page">
      <!-- header area  -->

      <div>
        <!-- doctor information  -->
        <div class="row p-5 pb-0">
          <div class="col-6">
            <h2 class="m-0">{{$prescription->doctor->name}}</h2>
            <p class="m-0"><b>{{$prescription->doctor->degrees}}</b></p>
            <p class="text-danger m-0">{{$prescription->doctor->specialist}}</p>
            <p class="m-0">SENIOR CONSulTANT</p>
            <b>
              <p class="text-primary m-0">{{$prescription->doctor->consultant_of_college}}</p></b
            >
          </div>
          <div class="col-6 text-end">
          <button id="printBtn" onclick="window.print()" class="btn btn-primary float-right">Print</button>
          </div>
        </div>
        <!-- doctor end information  -->

        <!-- prescriptions information  -->
        <div class="row px-5 pb-2">
          <div class="col-4">
            <img
              style="height: 30px; width: 250px"
              src="https://static.vecteezy.com/system/resources/previews/001/199/360/non_2x/barcode-png.png"
              alt=""
            />
            <p class="m-0"><b>Prescription ID</b> : 02</p>
          </div>
          <div class="col-4 text-center">
            <p><b>Total Visit</b> -01</p>
          </div>
          <div class="col-4 text-end pt-3">
            <span
              class="inline-block bg-light rounded-5 border border-2 border-gray p-2"
            >
              <b>UHID : 102-2022</b>
            </span>
          </div>
        </div>
        <!-- end prescriptions information  -->

        <!-- customer information -->
        <div
          class="row border px-5 border-start-0 border-end-0 border-1 border-dark"
        >
          <div class="col-5">
            <p class="m-0"><b>Name</b>: {{$prescription->patient->name}}</p>
            <p class="m-0"><b>Address</b>: {{$prescription->patient->address}}</p>
          </div>
          <div class="col-4">
            <p class="m-0"><b>Age</b>: {{$prescription->patient->age}} years</p>
            <p class="m-0"><b>Mobile</b>: {{$prescription->patient->mobile}}</p>
          </div>
          <div class="col-3">
            <p class="m-0"><b>Sex</b>: {{$prescription->patient->sex}}</p>
            <p class="m-0"><b>Date</b>: {{ date("Y-m-d")}}</p>
          </div>
        </div>
        <!--  end customer information -->

        <div class="d-flex">
          <!-- left side info  -->
          <div
            class="col-4 ps-5 border border-top-0 border-bottom-0 border-start-0 border-right-1"
          >
            <!-- single details  -->
            <div class="mt-2">
              <p
                class="border mb-0 border-gray border-end-0 border-start-0 border-top-0"
              >
                <b>Patient Details</b>
              </p>
              <div>
                <div>
                  •
                  <small>Blood Group: <span class="text-danger">O+</span></small>
                </div>

                <div>• <small>Marital Status: Married 5yr</small></div>
                <div>• <small>Occupation House wife</small></div>
                <div>• <small>Religion - Muslim</small></div>

              </div>
            </div>
            <!-- single details  -->
            <div class="mt-2">
              <p
                class="border mb-0 border-gray border-end-0 border-start-0 border-top-0"
              >
                <b>Chief Complaints</b>
              </p>
              <p>{{$prescription->cc}}</p>
            </div>
            <!-- single details  -->
            <div class="mt-2">
              <p
                class="border mb-0 border-gray border-end-0 border-start-0 border-top-0"
              >
                <b> Diagnosis</b>
              </p>
              <p>{{$prescription->diagnosis}}</p>
            </div>
            <div class="mt-2">
              <p
                class="border mb-0 border-gray border-end-0 border-start-0 border-top-0"
              >
                <b>Secondary Diagnosis</b>
              </p>
              <p>{{$prescription->sec_diagnosis}}</p>
            </div>
            <!-- single details  -->
            <div class="mt-2">
              <p
                class="border mb-0 border-gray border-end-0 border-start-0 border-top-0"
              >
                <b>Secondary Diagnosis 2</b>
              </p>
              <p>{{$prescription->sec_dx2}}</p>
            </div>
            <!-- single details  -->
            
            <!-- single details  -->
            <div class="mt-2">
              <p
                class="border mb-0 border-gray border-end-0 border-start-0 border-top-0"
              >
                <b>Investigations</b>
              </p>
              <ul>
                <li>Pt. Type: {{$prescription->mem_type}}</li>
                <li>Edu Year: {{$prescription->education}}</li>
                <li>SBP (mm of Hg): {{$prescription->sbp}}</li>
                <li>DBP (mm of Hg): {{$prescription->dbp}}</li>
                <li>Satu. (%): {{$prescription->oxygen}}</li>
                <li>Pulse (/min): {{$prescription->pulse}}</li>
                <li>Temp. (°F): {{$prescription->temp}}</li>
                <li>Edema: {{$prescription->edima}}</li>
                <li>Anemia: {{$prescription->anemia}}</li>
                <li>Jaundice: {{$prescription->jaundice}}</li>
                <li>Weight (Kg): {{$prescription->weight}}</li>
                <li>Height (cm): {{$prescription->height}}</li>
                <li>BMI: {{$prescription->bmi}}</li>
                <li>Bl. Gr.: {{$prescription->blgr}}</li>
                <li>Heart: {{$prescription->heart}}</li>
                <li>Lungs: {{$prescription->lungs}}</li>
                <li><strong>Disease History:</strong></li>
                <li>Diabetes: {{$prescription->diabeties}}</li>
                <li>HTN: {{$prescription->hp}}</li>
                <li>IHD: {{$prescription->ihd}}</li>
                <li>Stroke: {{$prescription->strk}}</li>
                <li>COPD: {{$prescription->copd}}</li>
                <li>Cancer: {{$prescription->cancer}}</li>
                <li>CKD: {{$prescription->ckd}}</li>
                <li><strong>Behavioural History::</strong></li>
                <li>Salt: {{$prescription->salt}}</li>
                <li>smoke: {{$prescription->smoke}}</li>
                <li>Smoking: {{$prescription->smoking}}</li>
            

              </ul>
            </div>
          </div>
          <!-- end left side info  -->

          <!-- right side  -->
          <div class="col-8 d-flex flex-column justify-content-between ms-4">
            <!-- medicine  -->
            <div class="mt-2">
              <!-- rx  -->
              <h1>R<span class="fs-4">x</span></h1>
              <!-- rx  -->
              <ol class="pe-5 me-4">
                @foreach($prescription->medicines as $med)
                <li>
                  <h5 class="p-0 m-0">{{$med->medicines}}</h5>
                  <div class="d-flex justify-content-between">
                    <p class="fs-6">{{$med->frequencies}} {{$med->quantities}} {{$med->quantity_types}} {{$med->eatingtimes}} </p>
                    <div class="d-flex justify-content-end">
                      <p class="mx-2">---------------------</p>
                      <p>{{$med->eatDuration}} month/Day</p>
                    </div>
                  </div>
                </li>
                @endforeach
                
              </ol>
            </div>
            <!-- medicine  -->

            <!-- Advice -->
            <div>
              <h4>Advice</h4>

              <ol>
                @foreach($prescription->advice as $adv)
                <li>{{$adv->name}}</li>
                @endforeach
                
              </ol>
            </div>
          </div>
          <!-- Advice -->
          <!-- end of  right side   -->
        </div>
      </div>

      <!-- footer  -->
      <small>
        <div>
          <p class="text-center m-0 text-danger fw-bold">
            {{$prescription->next_meet}} {{$prescription->meet_day}} পর রিপোর্টসহ দেখাতে আসবেন
          </p>
          <div class="px-5 bg-light pt-1 pb-3">
            <!-- chamber details  -->
            <div class="row align-items-center">
              <div class="col-5">
                <p class="mb-0">প্রতি শুক্রবার বিকাল ৩ টা থেকে রাত ৮ টা</p>
                <p class="mb-0">শামসুল আলম চৌধুরী মার্কেট,</p>
                <p class="mb-0">চৌহমুহনী, পেকুয়া কক্সবাজার</p>
                <p class="mb-0 text-primary">সিরিয়ালের জন্য:</p>
                <p class="mb-0 text-danger fw-bold">
                  ০১৮৮-৮৮৮-২২১, ০১৮৮-৮৮৮-২২১
                </p>
              </div>
              <div class="col-2">
                <img
                  class="img-fluid"
                  src="data:image/png;base64,"
                  alt=""
                />
              </div>
              <div class="col-5 text-end">
                <p class="mb-0">প্রতি শুক্রবার বিকাল ৩ টা থেকে রাত ৮ টা</p>
                <p class="mb-0">(মঙ্গলবার ও শুক্রবার বন্ধ)</p>
                <p class="mb-0">রুম নঙ- 319</p>
                <p class="mb-0">Health Care institure Name</p>
                <p class="mb-0">Health Care institure Name</p>
              </div>
            </div>
          </div>
        </div>
      </small>
    </div>

</div>


@endsection

@push('js')
   <!-- DataTables  & Plugins -->
    <script src="{{asset('backend/plugins/datatables/jquery.dataTables.min.js')}}"></script>
    <script src="{{asset('backend/plugins/datatables-bs4/js/dataTables.bootstrap4.min.js')}}"></script>
    <script src="{{asset('backend/plugins/datatables-responsive/js/dataTables.responsive.min.js')}}"></script>
    <script src="{{asset('backend/plugins/datatables-responsive/js/responsive.bootstrap4.min.js')}}"></script>
    <script src="{{asset('backend/plugins/datatables-buttons/js/dataTables.buttons.min.js')}}"></script>
    <script src="{{asset('backend/plugins/datatables-buttons/js/buttons.bootstrap4.min.js')}}"></script>
    <script src="{{asset('backend/plugins/jszip/jszip.min.js')}}"></script>
    <script src="{{asset('backend/plugins/pdfmake/pdfmake.min.js')}}"></script>
    <script src="{{asset('backend/plugins/pdfmake/vfs_fonts.js')}}"></script>
    <script src="{{asset('backend/plugins/datatables-buttons/js/buttons.html5.min.js')}}"></script>
    <script src="{{asset('backend/plugins/datatables-buttons/js/buttons.print.min.js')}}"></script>
    <script src="{{asset('backend/plugins/datatables-buttons/js/buttons.colVis.min.js')}}"></script>
    


@endpush